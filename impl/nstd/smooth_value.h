...
namespace nstd
{
	...
	public:
		...
		struct target_external_tag
		{
		};

		struct target_internal_tag
		{
		};
	};
	...
	public:
		...
		Q& set_target( )
		{
			auto target = std::make_unique<Q>( );
			auto out    = target.get( );
			target_val_ = std::move(target);
			return *out;
		}

		target_external& set_target(target_external_tag)
		{
			return set_target<target_external>();
		}

		target_external& set_target(target_internal_tag)
		{
			return set_target<target_internal>();
		}
