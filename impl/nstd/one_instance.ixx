...
template <typename T, size_t Instance, class Base>
class instance_of_t
{
  public:
...
export namespace nstd
{
    ...
    struct one_instance_getter
    {
        ...
        template <size_t Instance>
        one_instance_getter(const std::in_place_index_t<Instance>) : item_()
        {
        }
    ...
    class one_instance_getter<std::unique_ptr<T, D>>
    {
      public:
        ...
        template <size_t Instance>
        one_instance_getter(const std::in_place_index_t<Instance>) : item_(std::make_unique<T>())
        {
        }
    ...
    class one_instance
    {
      ...
      public:
        ...
        [[deprecated]] static void recreate() noexcept
        {
            _Initialized() = false;
    ...
    template <typename T, size_t Instance = 0>
    constexpr instance_of_t<T, Instance, nstd::one_instance<std::conditional_t<std::is_abstract_v<T>, T*, T>, Instance>> instance_of;
} // namespace nstd
